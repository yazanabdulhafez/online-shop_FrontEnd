...
const MyItems = () => {
  ...
  const showUpdateItemForm = (element) => {
    setId(element.id);
  ...
  const updateItem = (e) => {
    ...
    axios.put(`https://online-shop-ya.herokuapp.com/item/${id}`, dataBody)
      .then((response) => {
        console.log(response.data);
      })
      .catch(error => console.log(error));
            ...
            return (<Col key={index}>
              ...
              <Card style={{ width: '18rem', height: '30rem', marginTop: '2rem' }}>
                ...
                <Card.Footer style={{ display: 'flex', justifyContent: 'space-between' }}>
                  {isAuthenticated && <Button variant="primary" onClick={() => {
                    showUpdateItemForm(element);
